...
\begin{frame}
  ...
  needed to prove it.  This is never a formality---it usually requires new
  structures, and those structures often say something back to the
  physics.
  \bigskip
...
\begin{frame}
  Start with a 3d gauge theory: a compact group $G_c$ acting on a
  complex representation $N$, and the induced $\sigma$-model into
  $T^*N\cong \mathbb{H}\otimes_{\C} N$.  Call it $\mathcal{T}(G,N)$;
  it has $\mathcal{N}=4$ supersymmetry.
  \begin{block}{}
...
\begin{frame}
  ...
  These spaces include nilcones of semisimple Lie algebras, and behave
  like a new class of ``semisimple Lie algebras,'' vastly generalizing
  the Killing classification.
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item To a physicist: a duality.  To a mathematician: an outrageous
    list of conjectures pairing spaces with no visible relationship.
  \item From this perspective, 3d mirror symmetry generalizes the
    Langlands duality of semisimple Lie algebras.
  \end{itemize}
...
\begin{frame}
  ...
  \item {\bf Coherent sheaves:} explicit quiver-style descriptions of
    ...
    physics language, categories of boundary conditions---with
    tilting bundles generalizing Bezrukavnikov's noncommutative
    Springer resolution.
  \end{itemize}
  \begin{block}{}
...
\begin{frame}
  ...
  and $g=q^N$.
  \begin{block}{}
    Garoufalidis--Lauda--L\^e: for fixed $K$, these invariants satisfy
    recursion relations in the coloring $m$---the ``deformed
    $\hat{A}$-polynomial.''
  \end{block}
...
\begin{frame}
  ...
  \begin{itemize}
  \item The brane has its own invariant: {\bf knot contact homology}
...
\begin{frame}
  ...
  \item Physics predicted equalities between independently defined
    mathematical objects: Higgs $\leftrightarrow$ Coulomb; recursions
